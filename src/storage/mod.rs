...
mod project_list;
pub use self::project_list::{ProjectList, Projects, ProjectsByYear};
pub mod error;
pub mod repo;
...
pub fn get_storage_path() -> PathBuf {
    let storage_path = PathBuf::from(crate::CONFIG.var_get_str("path")).join(crate::CONFIG.var_get_str("dirs/storage"));
...
pub fn setup<L: Storable>() -> Result<Storage<L>, Error> {
    ...
    let working = crate::CONFIG
        .get_str_or("dirs/working")
        .ok_or_else(|| StorageError::FaultyConfig("dirs/working".into()))?;
    let archive = crate::CONFIG
        .get_str_or("dirs/archive")
        .ok_or_else(|| StorageError::FaultyConfig("dirs/archive".into()))?;
    let templates = crate::CONFIG
        .get_str_or("dirs/templates")
        .ok_or_else(|| StorageError::FaultyConfig("dirs/templates".into()))?;
...
impl<L: Storable> Storage<L> {
    ...
    pub fn try_new_with_git<P: AsRef<Path>>(
        root: P,
        working: &str,
        archive: &str,
        template: &str,
    ...
    pub fn health_check(&self) -> Result<(), Error> {
        ...
        if r.exists() && w.exists() && a.exists() && t.exists() {
            ...
            for f in &[r, w, a, t] {
                if !f.exists() {
                    log::warn!("{} does not exist", f.display())
                }
            }
    ...
    pub fn get_repository(&self) -> Result<&Repository, Error> {
        self.repository
            .as_ref()
            .ok_or_else(|| StorageError::RepoUninitialized.into())
    }
    ...
    pub fn create_dirs(&self) -> Result<(), Error> {
        ...
        if !self.templates_dir().exists() {
            fs::create_dir(self.templates_dir())?;
        }
    ...
    pub fn list_template_files(&self) -> Result<Vec<PathBuf>, Error> {
        ...
        let template_files = list_path_content(self.templates_dir())?
            .into_iter()
    ...
    pub fn create_project(
        ...
        let target_file = project_dir.join(slugged_name + "." + &L::file_extension());
    ...
    pub fn unarchive_project_dir(&self, archived_dir: &Path) -> Result<PathBuf, Error> {
        ...
        let parent_is_num = archived_dir
            .parent()
    ...
    pub fn list_empty_project_dirs(&self, directory: StorageDir) -> Result<Vec<PathBuf>, Error> {
        ...
        let projects = self
            .list_project_folders(directory)?
            .into_iter()
        ...
        let projects = match selection.into() {
            DirAndSearch(dir, ref search_terms) => {
                ...
                if projects.is_empty() {
                    anyhow::bail!(StorageError::NothingFound(
...
impl<P: Storable> fmt::Debug for Storage<P> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let storage = self.root_dir();
        let working = self.working_dir();
        let archive = self.archive_dir();
        let template = self.templates_dir();
        write!(
            f,
