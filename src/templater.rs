...
use std::{collections::HashMap, error::Error, fmt, fs, io, ops::Deref, path::Path};
...
impl<U: Deref<Target = str>> IsKeyword for U {
        ...
        F: Fn(&str) -> String,
    {
        Regex::new(REGEX)
            .expect("broken regex")
            .replace_all(self, |caps: &Captures<'_>| closure(caps.get(1).unwrap().as_str()))
            .into()
    }
}
...
impl Error for TemplateError {
    fn description(&self) -> &str {
        "The template was not filled completely."
    }
...
mod test {
    ...
    fn not_complete() {
        ...
        assert_eq!(
            filled_in.unwrap_err(),
            super::TemplateError::Incomplete(vec![String::from("ATTR"), String::from("SUBJ")])
        )
    }
}
